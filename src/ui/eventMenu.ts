...
export class EventMenu {
  ...
  show(event: TimelineEvent): void {
    ...
    this.parentTriggerText.textContent = currentParent ? currentParent.name : 'None (top level)';
  ...
  private showParentFlyout(anchor: HTMLDivElement): void {
    ...
    noneRow.className = 'parent-flyout-row' + (currentParent === null ? ' active' : '');
    ...
    this.buildFlyoutTree(flyout, candidates, currentParent, ancestorSet, anchor);
  ...
  private buildFlyoutTree(
    ...
    while (i < candidates.length) {
      ...
      while (childEnd < candidates.length && candidates[childEnd].depth > c.depth) {
        childEnd++;
      }
      ...
      row.className = 'parent-flyout-row' + (c.event === currentParent ? ' active' : '');
      ...
      if (hasChildren) {
        ...
        this.buildFlyoutTree(childContainer, childCandidates, currentParent, ancestorSet, anchor);
