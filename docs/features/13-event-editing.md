...
The event menu appears at the bottom of the screen, next to the event list panel. It is only visible when an event is selected (on canvas or from the events list). Selecting a different event updates the menu content.

The menu follows the same collapsible pattern as other panels: title bar at bottom (`column-reverse`), animated open/close via `max-height` transition, chevron rotation. Body scrolls when content is tall. The header has `cursor: grab` — draggable to swap position with the event list panel (see Feature 06).

### Sticky open state

The menu's collapsed/expanded state (`wantOpen`) is independent of selection. Clicking the header toggles the intent; this intent persists across event selections.

### Retained deselect

When `wantOpen` is true and the user clicks blank space (deselecting on the canvas), the menu stays open showing the last selected event. All editing controls remain functional — the user can continue editing even though no event is highlighted on the canvas. This avoids the jarring close/reopen cycle when clicking between events.

When the user explicitly collapses the menu header while in this retained state, the menu fully clears to "Nothing selected" and disables.

Destructive operations (delete event, import/replace data, clear all) always force-hide the menu regardless of `wantOpen`.
...
6. **Color section** — hover-triggered flyout with a 5-column swatch grid (14 colors + default). Clicking a swatch applies the color immediately.
7. **Parent section** — hover-triggered flyout to reparent the event. Shows a collapsible event tree.
8. **Export button** — exports the event (and descendants) as JSON.
9. **Delete button** — destructive action with confirmation dialog.
