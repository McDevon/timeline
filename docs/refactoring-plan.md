...
- **Add tests for untested pure functions** — 68 new tests across 4 test files: state.test.ts (eventToPath/pathToEvent round-trips, 16 tests), undo.test.ts (UndoManager push/undo/redo/coalescing/stack limits, captureSnapshot, resolvePathSet, 22 tests), hitTest.test.ts (point/range hit, nesting priority, scroll offset, 13 tests), reorder.test.ts (computeDropIndex, findSiblingInfo, sibling layout items, parent layout item, buildRefPositions, 17 tests). Total: 239 tests.
